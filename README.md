...
- 🔒 **%100 Gizlilik ve İnternetsiz Çalışma:** Tüm veri ve yapay zekâ modelleri yerelde çalışır, hiçbir veri dış sunuculara gönderilmez.
- 🇹🇷 **%100 Türkçe Kod ve Arayüz:** Tüm modül isimleri, fonksiyonlar, veritabanı şeması ve arayüz Türkçe.
...
- **Kullanıcı Arayüzü (UI):** [Streamlit](https://streamlit.io/) (`uygulama.py` — Özel CSS temalı)
...
- **Veritabanı:** [aiosqlite](https://github.com/omnilib/aiosqlite) (C tabanlı Cosine Similarity UDF entegrasyonu ile — `veritabani.py`)
- **Döküman İşleme:** `pypdf` & Akıllı Parçalama (`yukleyici.py`)
...
## 📁 Proje Yapısı (Türkçe Modüller)

```
microsoft-rag/
├── uygulama.py             # Streamlit ana arayüz uygulaması (Türkçe)
├── app.py                  # Takma ad (Alias) giriş noktası
├── requirements.txt        # Bağımlılıklar
├── test_faz1.py            # Entegrasyon test senaryosu
├── core/
│   ├── __init__.py
│   ├── veritabani.py       # Asenkron SQLite veritabanı & Cosine Similarity UDF
│   ├── yukleyici.py        # Döküman okuma, parçalama ve vektörleştirme
│   ├── arama_motoru.py     # Semantik & Hibrit arama motoru
│   └── ureteci.py          # Yerel LLM streaming yanıt ve özet üretimi
└── README.md
```
